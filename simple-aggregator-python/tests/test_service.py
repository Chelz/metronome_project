...
def domain_model_return_df():
    df = pandas.DataFrame.from_records(
        [
            {"num_events": 2, "hour_bucket": "2021-01-01 00:00:00"},
            {"num_events": 2, "hour_bucket": "2021-01-01 01:00:00"},
        ]
    )
    return df
...
def test_service(domain_model_return_df):
    with patch.object(
        ...
        "count_events",
        return_value=domain_model_return_df,
    ) as mock_method:
        ...
        m = ta.validate_json(
            domain_model_return_df.to_json(orient="records", date_format="iso")
        )

        expected_response = SimpleAggregatorResponseItems(data=m, customer_id="1")
