...
def client():
    d = {"table_name": "events3", "data_path": "data/*.csv"}
...
def test_get_with_customer_id(client):
    response = client.get(
        "/events/b4f9279a0196e40632e947dd1a88e857?start_timestamp=2021-03-01%2000:00:00&end_timestamp=2021-03-01%2010:00:00"
    )
...
def test_bad_request_with_no_path_paramn(client):
    response = client.get(
        "/events/?start_timestamp=2021-03-01%2000:00:00&end_timestamp=2021-03-01%2010:00:00"
    )
