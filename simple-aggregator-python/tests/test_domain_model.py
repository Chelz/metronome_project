...
def domain_model(input_pandas_df):
    domain_model = CountEventsByCustomerIdWithDuckDB(
        "", "events", bootstrap_from_filepath=False
    )
...
def test_count_events_with_valid_customer_id(domain_model):
    response = domain_model.count_events(
        customer_id=1,
        start_timestamp="2023-01-01 00:00:00",
        end_timestamp="2023-01-01 00:02:00",
    )
    ...
    response = domain_model.count_events(
        customer_id=1,
        start_timestamp="2023-01-01 00:00:00",
        end_timestamp="2023-01-01 01:20:00",
    )
    response_dict = _convert_to_list(response)
    assert len(response_dict) == 1
...
def test_count_events_with_invalid_date_ranges(domain_model):
    response = domain_model.count_events(
        customer_id=5,
        start_timestamp="2023-01-02 00:00:00",
        end_timestamp="2023-01-02 00:02:00",
    )
    response_dict = _convert_to_list(response)
    assert len(response_dict) == 0
